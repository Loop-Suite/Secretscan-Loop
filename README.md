...
- **Nothing in this tool ever prints, logs, or serializes a raw secret value.** Every `Candidate` carries only a masked preview (`scanners::mask`): values ≤8 characters are fully masked; longer values keep `clamp(n/5, 3, 6)` characters at each end and mask the middle, e.g. `AKIA****...****3F2Q (len=20)`. Every occurrence on a context line is masked, across all rules/sources that matched that line, before the line goes anywhere — including into an LLM prompt.
