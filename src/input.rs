...
fn count_files(target: &Path) -> usize {
    walkdir::WalkDir::new(target).into_iter().filter_map(|e| e.ok()).filter(|e| e.file_type().is_file()).count()
}

pub fn normalize(target: &Path, requirements_path: &Option<std::path::PathBuf>, scope: &ScanScope) -> Result<Input> {
    anyhow::ensure!(target.exists(), "scan target does not exist: {}", target.display());
    ...
    Ok(Input { target: target.to_path_buf(), candidates, files_scanned, requirements })
}
